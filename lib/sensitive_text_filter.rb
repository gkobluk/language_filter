...
require 'pathname'
require 'sensitive_text_filter/error'
require 'sensitive_text_filter/version'

module SensitiveTextFilter
  class Filter
    ...
    DEFAULT_MATCHLIST = File.dirname(__FILE__) + "/../config/matchlists/custom_match.txt"
    ...
    def validate_list_content(content)
      case content
      when Array    then content.all? {|c| c.class == String} || raise(SensitiveTextFilter::EmptyContentList.new("List content array is empty."))
      when String   then File.exists?(content)                || raise(SensitiveTextFilter::UnkownContentFile.new("List content file \"#{content}\" can't be found."))
      when Pathname then content.exist?                       || raise(SensitiveTextFilter::UnkownContentFile.new("List content file \"#{content}\" can't be found."))
      when Symbol   then
        case content
        when :default, :email, :custom_match, :url, :phone, :address then true
        else raise(SensitiveTextFilter::UnkownContent.new("The only accepted symbols are :default, :email, :custom_match, :url, :phone and :address"))
        end
      else raise SensitiveTextFilter::UnkownContent.new("The list content can be either an Array, Pathname, or String path to a file.")
      end
    end
    ...
    def set_list_content(list,options={})
      case list
      when :email      then load_list File.dirname(__FILE__) + "/../config/#{options[:folder] || "matchlists"}/email.txt"
      when :url then load_list File.dirname(__FILE__) + "/../config/#{options[:folder] || "matchlists"}/url.txt"
      when :phone       then load_list File.dirname(__FILE__) + "/../config/#{options[:folder] || "matchlists"}/phone.txt"
      when :address  then load_list File.dirname(__FILE__) + "/../config/#{options[:folder] || "matchlists"}/address.txt"
    ...
    def replace(word)
      ...
      when :url then then word.gsub(/[^bcdfghjklmnpqrstvwxyz]/i, '*')
